...
- データソースの利用規約・商用可否は [data_catalog.md](../sources/data_catalog.md) を参照。
...
| `population_prefecture`            | 男女別人口 都道府県 世紀 companion（系統B・1920〜2020）          | 0003410379  |
| `population_prefecture_timeseries` | 都道府県 世紀 companion の配布正典（1920〜2020＋2025速報, 派生） | 合成        |

出典はいずれも「政府統計の総合窓口(e-Stat)」。

> **都道府県時系列は系統B世紀 companion（1920〜）**：`population_prefecture_timeseries` は
> 旧・空間rollup 版（市区町村旗艦→県・1980〜）から、系統B の長期時系列表
> **0003410379「男女別人口及び人口性比 － 全国，都道府県（大正9年～令和2年）」** へ張り替えた
> （戦略B＝fact 独立＋companion。5歳階級の `population_by_age5_prefecture` と同型）。
> 出力シェイプ（47都道府県・全国行なし＝全国は Σ県で復元・2025速報を splice）は旧版と同一で、
> **到達年だけが 1980→1920 へ伸びる**。1980-2020 の重複年は旧・市区町村rollup と一致
> （唯一の差＝東京都1980の +37人＝特別区部の区未定分。系統B側が区未定分を含む正しい県総数）。
