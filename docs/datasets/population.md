...
| `population_timeseries` | 男女別人口の時系列（2000〜2020を結合, 派生データセット） | 合成        |

出典はいずれも「政府統計の総合窓口(e-Stat)」。

```bash
uv run poe run population_2020         # 2020 単年
uv run poe run population_timeseries   # 2000〜2020 時系列
```
...
3. 平成大合併は 2005 の調査時点(2005/10)で大半が完了済みのため、2005 起点では合併が捉えにくい
   （2005にあり2020に無いコードは93件）。**2000 年を追加すると合併前の自治体が大幅に増え、畳み込み（連続時系列）の
   価値が本領を発揮する** → 地域マスタで 2000〜2020 を接続済み（人口保存 全年 diff=0・孤児0 を検証）。
