...
>
> **1985 遡及の実例（overrides 不要）:** 1985 年（0003412414, 1990/1995 と同型表）を追加しても**孤児 0**（新規 overrides ゼロ）。
> 昭和60年の廃置分合も parsed events ＋ 既存 overrides だけで 1985→2020 が畳めた。
> 人口保存 1985=121,048,923 diff=0、局所検算: さいたま市 11100 の 1985 = 浦和 377,235+大宮 373,022+岩槻 100,903+与野 71,597 = 922,757 一致。
>
> **1980 遡及の実例（overrides 1 行＋原資料の 37 人差）:** 1980 年（0003412413, 1985 と同型表・ID 連番）を追加すると孤児 1 件（塩原町）。
> 上記②と同型の**郡区域変更**で、`09381→09410`(塩原町。1980 は塩谷郡 09381、1985〜は那須郡 09410 へコード変更。以降 09410→09213 那須塩原市=2005 新設合併は parsed で解決) を 1 行補うと **孤児 0**。
> 局所検算: 那須塩原市 09213 の 1980 = 黒磯 46,574＋西那須野 28,807＋塩原 10,055 = 85,436 一致。
> ただし 1980 のみ人口保存に **37 人差**が残る（東京都特別区部の「区未定分」＝上記の原資料特性。孤児とは無関係で override では解消しない）。
