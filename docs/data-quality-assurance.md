...
| **I1** | industry(就業者数)→県rollup == industry_prefecture             | 県×year×産業       | A×B  |
| **O1** | occupation(就業者数)→県rollup == occupation_major12_prefecture | 県×year×職業       | A×B  |
| **L1** | labor_force(人口)→県rollup == labor_force_prefecture           | 県×year×労働力状態 | A×B  |

> C1〜C5 は person（総人口）の三角測量。**H1/H2・F1/F2 は世帯（household）、I1/O1/L1 は就業（employed／population） universe** に同じ「ミクロ回次別→県 rollup == マクロ回次跨」オラクルを広げたもの（突合軸は各 universe の測度＝世帯数・就業者数・人口）。
...
- **I1/O1/L1**（就業系 industry/occupation/labor_force ミクロ回次別→県 rollup == 各 `_prefecture` マクロ回次跨）も同型。就業状態等基本集計の大分類は**悉皆カウント（標本でない）**ゆえ回次別と回次跨が県レベルで一致。ミクロは **2015 のみ着手**（唯一「男女×大分類×市区町村」の軽量 2 次元 marginal が揃う年）＝**2015 は厳密 diff=0**（I1=987／O1=611／L1=282 キー）、県跨マクロが持つ他の収録年（industry/occupation=2005/2010/2020・labor_force=1950〜2020 の 5 年間隔）は**ミクロ未収録＝scope_out**。★着手順 2020→2010→1995-2005 で順次外す（分類断層＝産業 3 体系 15/19/20 区分・職業 2 体系 10/12 区分ゆえ 2005 以前は別マップ／別セグメント）。
...
- `crossfact-check`: 「クロスファクト検算」＋日本人スライスの上界/保存則＋地理保存を実データで走らせ年別 status に分類（自動化済みは C1 / C2 / C3 / C4 / C5 / H1 / H2 / F1 / F2 / I1 / O1 / L1 / J1〜J3／G＝全国==Σ県）。
...
- **カテゴリ内訳保存（Σカテゴリ==総数）の実データゲート**
  - 現状: 測定量の非負は `sanity-check` が実データでゲート化済（不詳導出のはみ出し＝負値を捕捉）。一方「Σカテゴリ == 総数行」の内部整合は、不詳を導出注入するファクト（family_type/labor_force マクロ）は構成上ゼロで閉じる（＝残差の非負を `sanity-check` が見れば十分）が、注入しないファクト（households/industry/occupation_major10/12・**就業系ミクロ市区町村 industry/occupation/labor_force**＝いずれも「分類不能／不詳」が実カテゴリで注入不要）は真の不変条件でありながら実データでは未ゲート（`tests/cleaners/` の合成 fixture テスト止まり）。地理保存 G はカテゴリごとの全国==Σ県を見るだけで Σカテゴリ==総数 は突かない
    - **就業系ミクロ市区町村（2015）はアドホック実測で違反0を確認済**（市区町村×性別×年 5,220 群すべてで 総数==Σ大分類・null を0扱い）。加えて **I1/O1/L1 の県 rollup diff=0 が「null＝秘匿ではなく該当なし(=0)」を推移的に証明**（秘匿=非ゼロなら rollup が県マクロを下回り diff≠0 になるはず）。ただし常設 spec 化は未了＝下記方針に合流。
...
- **単年 Stitched の合併畳込が未発火**
  - 現状: 就業系ミクロ市区町村は 2015 単年で着手＝縫合対象が1年のみゆえ `aggregate_to_base`（base_year=最新年=2015）の合併畳込が実質恒等（no-op）。市区町村 Stitched の要である**合併補正のロジックは、複数回次が揃う（2020 追加）まで実データで発火しない**＝I1 の県 rollup diff=0 も「合併をまたがない単年一致」を見ているに留まり、畳込そのものは未ストレステスト（age5/households/family_type は複数回次で発火済み＝既に検証網内）
  - 対応方針: 着手順どおり 2020 を追加した時点で県跨ぎ合併（山口村 長野→岐阜 等）を含む畳込が発火し、I1/O1/L1 が `mode="conservation"`＋必要なら known_diff pin で回帰ガードする（H1/F1 と同じ経路）。それまでは 2015 単年の一致で暫定担保
- **新年度スキーマ**
