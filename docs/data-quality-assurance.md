...
#### ② クロスファクト検算（`known_pins.py` の `CROSSFACT_C4`/`CROSSFACT_C5`＝年→Σ\|diff\|）

別プロダクト間の集計差。近年（2010〜2020）は厳密 diff=0 で、**遡及年のみ**残る差を年ごとに Σ\|diff\| で pin する（全件の値は `known_pins.py` の `CROSSFACT_C4`/`CROSSFACT_C5`、`area/specs.py` の該当 spec がこれを参照）:
