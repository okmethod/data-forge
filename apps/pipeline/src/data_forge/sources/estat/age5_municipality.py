...
対象＝1980/1985/1990/1995/2000/2005/2010/2015/2020。2000/2005 は各歳の巨大表（0000032965/0000033783）を使い、
その年齢軸に内包される 5歳階級の再掲コードだけを拾う（5歳階級の2表分割 0000032807/0000032969 は県庁所在市・特別区が
どちらにも入らない穴があり全国が約476万人不足するため不採用＝census_source_tables §3-3）。巨大ゆえ
datasets 側でサーバ側絞り込み（全域＋5歳コード）を掛ける。各年の生表は年齢5歳階級のほかに国籍・出生の月・
全域/人口集中地区(DID) の軸を持ち、**どの catNN が男女／年齢か（軸割当）もコード体系も年ごとに違う**
（特に 2015 は男女=cat03・年齢=cat02 と入れ替わり、getMetaInfo とも食い違う）。よって cleaner は
年ごとに `clean_<year>` を持ち、共通の `_clean` へ tab（無い年は None）・軸列名・コード写像・
「余分な軸を総数コードで潰す」フィルタ・国籍の扱い（定数注入 or 軸写像）を渡す。

★**国籍(nationality)軸を持つ**（総数=0 / 日本人=1）。国勢調査は 1985→1990 で市区町村×5歳階級の集計基底を
切替えたため、市区町村フル粒度で取れる基底が年で非対称（census_source_tables §3-3 のマトリクス）:
    - 総数のみ … 1980/1985（国籍軸なしの総人口表＝日本人を抜けない）
    - 両方（別表を union）… 1990/1995。日本人は 5歳階級表 006（clean_1990/1995）、総数は各歳表 00401
      （0000031401/0000032219＝clean_1990_1995_total）という**表形式の非対称**（当初「総数は市区町村フルに無い」
      と誤認したが、各歳表 00401 に市区町村フルで実在＝census_source_tables §3-3 の訂正）。同年に2 Dataset を持つ。
    - 両方（単一表）… 2000/2005/2010/2015/2020（国籍軸あり＝総数・日本人を両方出す＝重なり年で検証可能）
    - 2005 は各歳表 0000033783（2000 の 0000032965 と同型・cat04 だけ男女×出生月＝000/001/006）を `clean_2005` で取得。
companion（系統B・age5.py）は総人口専用で nationality 軸を持たない（別 stem＝別ファイルで物理非同居・
既に age_class_code 体系も異なる＝loose family）。旗艦のみが nationality を担う。

既存 age5（系統B・県表が85歳以上止まり）と違い、市区町村版は **100歳以上まで保持**する
（独自テーブル）。年齢不詳は各回表に実コードで存在する年はそのまま採る（2020=22/2010,2015=999/
1980=022/1985=023。ドキュメントの「1980 は 75歳止まり・不詳無し」は誤りで実測 23 区分＝2020 と同型）。
1990/1995/2005 は不詳コードそのものが無い（＝999 行が出ない。2005 は「年齢不詳を除く」表）。
age5 のような導出注入はしない。

出力スキーマ 12 列（age5 の10列＋nationality_code/nationality）。area_level は 1〜7・is_current で
level7 を識別。合併畳み込み（aggregate_to_base）は共有インフラに委ねる＝clean は全 area level を素直に出す。
grain は area×year×sex×nationality×age_class。
...
def clean_1990(tidy: pl.DataFrame) -> pl.DataFrame:
    ...
    総人口版は市区町村フル粒度に存在しない（census_source_tables §3-3）。年齢不詳コードは原表に無い。
...
def clean_1990_1995_total(tidy: pl.DataFrame) -> pl.DataFrame:
    ...
    （census_source_tables §3-3。日本人版は 5歳階級表 006、総人口はこの各歳表 00401 という表形式の非対称）。
...
def clean_2005(tidy: pl.DataFrame) -> pl.DataFrame:
    ...
    clean_2000 を流用し cat04 の男女コードだけ 000/001/006（男女の総数）に読み替える（出生の月クロスは sex_map
    非収載で自動除外）。年齢軸に 5歳階級の再掲コード（T01/200-219/500=100歳以上）を内包するので age_map で
    5歳階級だけ拾う（各歳は自動除外）。★この表は年齢不詳を除くため不詳行が無い（Σ5歳階級==総数 が成立）。
    国籍軸あり＝総数・日本人を両出し。全域(cat01=00700) で DID を落とす。政令市区(level5)や集計行は
    aggregate_to_base のアトム抽出（muni_levels={4,6}）が落とす。
    """
    return _clean(
