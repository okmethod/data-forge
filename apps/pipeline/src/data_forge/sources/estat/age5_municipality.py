"""国勢調査 年齢（5歳階級）×男女別人口 市区町村版（population_by_age5 のミクロ系列）のクレンジング。

各回の基本集計（回次別）の市区町村5歳階級表。同じ table_name "population_by_age5" に同居する全国／都道府県のみの世紀
マクロ（回次跨・単一ID・1920-2020／key は _national/_prefecture/_prefecture_timeseries）と対をなし、
市区町村（旧市区町村 level7 を含む年もある）まで下りる。対象＝1980/1985/1990/1995/2000/2005/2010/2015/2020。
表ID・軸割当・国籍/不詳カバレッジの年別マトリクス、2000/2005 の各歳表採用理由
（5歳階級2表 0000032807/0000032969 の穴で不採用）は
**docs/sources/estat-census-catalog.md「年齢（5歳階級）」節が正典**（ここには再掲しない＝ドリフト防止）。

**軸の採否原則**: grain に載せる軸＝「保存則が閉じる・下流需要がある」もの
（area / sex / age_class / nationality）のみ。
他のクロス軸（全域・人口集中地区(DID)・出生の月）は総数コードで潰す（＝出力に残さない）。
★**どの catNN が男女／年齢か（軸割当）もコード体系も年ごとに違う**ため
cleaner は年別に `clean_<year>` を持ち、共通の `_clean` へ tab（無い年は None）・軸列名・コード写像・
「余分な軸を総数コードで潰す」フィルタ・国籍の扱い（定数注入 or 軸写像）を渡す。
**マップは必ず getStatsData の実コードで確定する**
（getMetaInfo と食い違う年がある＝特に 2015 は男女=cat03・年齢=cat02 と入替）。
2000/2005 は各歳の巨大表（0000032965/0000033783）から 5歳階級の再掲コードだけを拾い、
巨大ゆえ datasets 側でサーバ側絞り込みを掛ける。

★**国籍(nationality)軸を持つ**（総数=0 / 日本人=1）。市区町村フル粒度で取れる基底が年で非対称
（総数=1980-2020 全年・日本人=1990 以降。1990/1995 は総数=各歳表00401・日本人=5歳階級表006 の別表 union）で、
取れるセルだけ埋める（詳細マトリクスは「年齢（5歳階級）」節）。マクロ（回次跨・age5.py）は総人口専用で
nationality 軸を持たない（別 stem＝別ファイルで物理非同居・age_class_code 体系も異なる＝loose family）。
ミクロ系列のみが nationality を担う。

既存 age5（回次跨・県表が85歳以上止まり）と違い、市区町村版は **100歳以上まで保持**する（独自テーブル）。
**不詳の扱い原則＝導出注入は総数と階級が同一母集団だと保証できる表に限る**。回次跨 age5 / 3区分時系列は同一母集団ゆえ
`総数−Σ` で不詳を注入するが、本ミクロ系列の各歳表は国籍カバレッジが非対称（総数=全年・日本人=1990+）で
総数と5歳階級の母集団保証が弱く、`総数−Σ` は真の不詳でなく基底差の残差を含みうる（＝偽の不詳を作る）。
よって本ミクロ系列は**不詳の実コードがある年はそのまま採り・無い年は注入しない**
（無い年＝1990/1995/2005。1990/1995 は不詳が原表になく Σ5歳==総数。2005 は不詳行こそ無いが
総数 T01 に5歳階級へ未分類の残差＝埋め込み不詳が残り Σ5歳 ≤ 総数＝crossfact J2 が known_diff で受容）。
この age5 との非対称はバグでなく上記原則の帰結（per-year の不詳コードは「年齢（5歳階級）」節）。

出力スキーマ 12 列（age5 の10列＋nationality_code/nationality）。area_level は 1〜7・is_current で
level7（旧市区町村・2020 のみ）を識別。合併畳み込み（aggregate_to_base）は共有インフラに委ねる＝
clean は全 area level を素直に出す。grain は area×year×sex×nationality×age_class。
出力 age_class_code は 5歳刻みの独自連番（100=総数 / 999=不詳。全コードは下記 AGE_CLASS）。
回次跨 age5 の飛び番（140/270 欠番）とは別体系だがテーブルが別なので衝突しない。
"""
