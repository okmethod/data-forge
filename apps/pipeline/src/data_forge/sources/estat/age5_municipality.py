...
各回の基本集計（系統A）の市区町村5歳階級表。同じ table_name "population_by_age5" に同居する全国／都道府県のみの世紀
companion（系統B・単一ID・1920-2020／key は _national/_prefecture/_prefecture_timeseries）と対をなし、
...
取れるセルだけ埋める（詳細マトリクスは「年齢（5歳階級）」節）。companion（系統B・age5.py）は総人口専用で
nationality 軸を持たない（別 stem＝別ファイルで物理非同居・age_class_code 体系も異なる＝loose family）。
旗艦のみが nationality を担う。

既存 age5（系統B・県表が85歳以上止まり）と違い、市区町村版は **100歳以上まで保持**する（独自テーブル）。
**不詳の扱い原則＝導出注入は総数と階級が同一母集団だと保証できる表に限る**。系統B age5 / 3区分時系列は同一母集団ゆえ
`総数−Σ` で不詳を注入するが、本旗艦の各歳表は国籍カバレッジが非対称（総数=全年・日本人=1990+）で
総数と5歳階級の母集団保証が弱く、`総数−Σ` は真の不詳でなく基底差の残差を含みうる（＝偽の不詳を作る）。
よって本旗艦は**不詳の実コードがある年はそのまま採り・無い年は注入しない**
...
系統B age5 の飛び番（140/270 欠番）とは別体系だがテーブルが別なので衝突しない。
"""
