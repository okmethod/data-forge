...
（0003414255、新分類区分・1995〜2020）を配布用の1枚テーブルへ整形する。households.py の
...
households と同じく2測定量（一般世帯数・一般世帯人員）を1行へ横並びに持つ。1世帯当たり人員(1390)は
世帯人員÷世帯数、世帯数割合(1930)は各類型÷総数で導出可能ゆえ捨てる（基底だけ持ち残りは導出）。

分類ツリー（family_type_code / @level）:
    100 総数(1)
    ├ 110 親族のみの世帯(2)
    │  ├ 120 核家族世帯(3) … 130/140/150/160(4)          葉4
    │  └ 170 核家族以外の世帯(3) … 180〜270(4)             葉10
    ├ 280 非親族を含む世帯(2)                               葉1
    ├ 290 単独世帯(2)                                       葉1
    └ 999 家族類型不詳(2)  ← 導出注入（下記）
「16区分」＝葉16個（130-160・180-270・280・290）。100/110/120/170 は小計。

**家族類型不詳の導出注入**（labor_force の労働力状態不詳・age5 の年齢不詳と同型）:
level-2 の直下区分は 110/280/290 だが、原資料の 総数(100) はこれらの和を上回る（2010〜2020 の
実データで残差＝不詳世帯が生じ、2020全国で約16万世帯）。不詳は cat01 に独立コードが無いため
**総数(100) − 親族のみ(110) − 非親族(280) − 単独(290)** として 999「家族類型不詳」を area×year 毎に
導出注入する。これで「110 + 280 + 290 + 999 == 100」が全地域・全年で恒等成立する（120/170 は 110 の
内訳＝再掲なので減算に含めない）。サブツリー保存則 110==120+170 / 120==130+…+160 / 170==180+…+270 は
原資料で恒等成立する（実データで全違反0を確認済み・検証は docs/data-quality-assurance.md）。

出力スキーマ（households の8列＋family_type_level の9列）:
    area_code(str) / area_name(str) / area_level(int) /
    family_type_code(str) / family_type(str) / family_type_level(int) /
