...
def _tidy_mixed() -> pl.DataFrame:
    ...
    return pl.DataFrame(
        [
            _row(tab="040", htype="100", value=100),
            _row(tab="040", htype="100", value=5, area="01000", area_name="北海道", level="2"),
        ]
    )
...
def test_scope_prefecture_drops_national():
    ...
    df = households.clean_households(_tidy_mixed(), scope="prefecture")
...
def test_scope_national_keeps_only_national():
    ...
    df = households.clean_households(_tidy_mixed(), scope="national")
