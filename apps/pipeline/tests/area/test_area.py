...
def test_conservation_known_diff_is_accepted():
    ...
    assert known_pins.KNOWN_DIFFS[1980] == 37
    fact = _fact([("01201", "A市", 1980, 100 - 37), ("01201", "A市", 2020, 150)])
    national = pl.DataFrame({"year": [1980, 2020], "sex_code": ["0", "0"], "population": [100, 150]})
    cons = reconcile.national_conservation(fact, national, known_diffs=known_pins.KNOWN_DIFFS).sort("year")
...
def test_conservation_unknown_diff_still_fails():
    ...
    cons = reconcile.national_conservation(fact, national, known_diffs=known_pins.KNOWN_DIFFS)
